...
def send_read_request(sock, sessionId):
	rq = Request()
	rq.type = READ
...
def send_connect_request(sock):
	...
	print("Send reqeust: %d" %rq.type)
	rq.session_id = 0
	rq.authorization.username = "test"
	rq.authorization.password = "test"
	print("")
	send_message(sock, rq)
	return get_connect_response(sock)

def get_connect_response(sock):
	""" Read a serialized response message from a socket.
	"""
	...
	print("Get response: %d" %msg.type)
	print("session id: %d" %msg.authorization.session_id)
	print("")
	return msg.authorization.session_id
...
if __name__ == '__main__':
	...
	sessionId = send_connect_request(sockobj)
	send_read_request(sockobj, sessionId)
	print("---------------------------")
